...
TC- O(N) where is N is length of the logs, SC- O(N)
...
started at the beginning of timestamp 3, and "1:end:2" means a function call with function ID 1 ended at the end of
...
class Solution:
    """
    *most optimized*
    Ideation- Stack, TC- O(N)
    The idea is to push the process when it starts and pop is when it ends from the stack. Except, there are caveats.
    1. New process starts when a process was already running - We save the time the current process ran before the new
    one comes in.
    2. Old process in stack resumes when the current is done - We update the start time of previous process in stack
    with the current process end time  + 1.
    """
    ...
    def exclusiveTime(self, n, logs):
        ...
        for log in logs:
            ...
            if typ == 'start':
                # a process was already running in stack. We store how much that process has run so far on cpu
                if len(st) > 0:
                    # previous process
                    pfid, ptyp, ptime = st[-1]
                    result[pfid] += (int(time) - ptime)
                st.append([int(fid), typ, int(time)])
            else:
                # current process
                cfid, ctyp, ctime = st.pop()
                result[cfid] += (int(time) - ctime + 1)
                # when a process ends, it resumes the previous process in the stack. To do so, we will change the start
                # time of the prev process to current end + 1
                if len(st) > 0:
                    pfid, ptyp, ptime = st[-1]
                    st[-1][2] = int(time) + 1
